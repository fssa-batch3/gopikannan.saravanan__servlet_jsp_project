            ...
            for (let i = 0; i < fundraiseArr.length; i++) {
                ...
                if (window.location.pathname === '/pinapp/index.jsp') {
                	 if ( percentage >= 40 && amount<expected_amount) {
                         ...
                         image_share.setAttribute("src", "<%= request.getContextPath() %>/assets/images/share (1).png");
                	...
                	if(amount<expected_amount){
                     ...
                     document.getElementById("inputId").addEventListener("input", () => {
                         for (let i = 0; i < cards.length; i++) {
                             const element = cards[i];
                             if (
                                 element.innerHTML.toLowerCase().includes(document.getElementById("inputId").value.toLowerCase())
                             ) {
                                 element.style.display = "block";
                             } else {
                                 element.style.display = "none";
